...
describe 'sweater weather api' do
  ...
  it 'returns roadtrip info' do
    roadtrip = File.read('./spec/fixtures/roadtrip_response.json')

    stub_request(:post, '/api/v1/road_trip').with(body: { origin: 'Denver,CO', destination: 'Pueblo,CO', api_key: "#{@user.api_key}" }.to_json, headers:  { "CONTENT_TYPE" => "application/json"  }).to_return(status: 200, body: roadtrip )
